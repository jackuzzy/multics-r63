...
// 03 00 00 01 2F EE B1 CB 54 00 00 D8 03 
	...
	buf[7] = 'M'^buf[1]^buf[2]^buf[3]^buf[4]^buf[5]^buf[6]; // Multics Checksum
	buf[8] = 'M'; // Multics ID
	//Port
	buf[9] = 0;
	buf[10] = 0;
	...
	if (received>0) {
		...
		switch(buf[0]) {
				case TYPE_REQUEST:
					...
					if ( !strcmp("MultiCS",peer->program) && (!strcmp("r50",peer->version)||!strcmp("r51",peer->version)) ) break;
					// Check CSP
					...
					while (peer) {
						if (peer->host->ip==recv_ip) {
							if (peer->port==port) { // Peer is found
								...
								// Check for Multics Checksum
								if ( buf[8]=='M' && buf[7]==('M'^buf[1]^buf[2]^buf[3]^buf[4]^buf[5]^buf[6]) ) {
									strcpy(peer->program,"MultiCS");
								}
...
*/
				...
				case TYPE_PINGRPL:
					...
					if (buf[8]!='M') break;
					if ( buf[7]!=('M'^buf[1]^buf[2]^buf[3]^buf[4]^buf[5]^buf[6]) ) break;
					// Get Peer
